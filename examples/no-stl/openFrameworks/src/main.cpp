...
public:
    ...
    void setup()
    {
        ...
        // echo back
        MsgPacketizer::subscribe(serial, recv_back_index, [&](std::vector<int> vv, std::map<std::string, float> mm)
        {
    ...
    void update()
    {
        ...
        std::vector<int> vo {1, 2, 3, 4};
        std::map<std::string, float> mo {{"one", ofGetFrameRate()}, {"two", 2.2f}, {"three", 3.3f}, {"four", 4.4f}};
        
        MsgPacketizer::parse();
        // this packet will be sent back to index = recv_direct_index
        MsgPacketizer::send(serial, send_direct_index, (int32_t)ofGetFrameNum(), 2.2f, "three");
        // this packet will be sent back to index = recv_back_index
        MsgPacketizer::send(serial, send_lambda_index, vo, mo);

        update_info << std::dec;
        update_info << "data : " << i << ", " << f << ", " << s << std::endl;
    }
